...
class MachineBloc extends Bloc<MachinesEvent, MachinesState> {
  ...
  MachineBloc(this._getMachinesUseCase) : super(MachinesStateInitial()) {
    //event for when we at first seek the machines
    on<OnMachinesRetrieving>((event, emit) async {
    ...
    on<OnNewMachine>((event, emit) async {
      List<MachineEntity> machines = [];
      if (state is MachinesRetrievingSuccess)
        machines = (state as MachinesRetrievingSuccess).machines;
    ...
    on<OnMachinesExpansionCollpased>((event, emit) {

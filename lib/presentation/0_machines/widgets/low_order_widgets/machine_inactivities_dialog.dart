...
class MachineInactivitiesDialog extends StatefulWidget {
  ...
  @override
  State<MachineInactivitiesDialog> createState() => _MachineInactivitiesDialogState();
}
...
class _MachineInactivitiesDialogState extends State<MachineInactivitiesDialog> {
      ...
      builder: (context, state) {
        if (!_automaticInitialized && state.hasMachine) {
          _continueCapacityController.text = state.continueCapacity > 0
              ...
              : '';
          _restMinutesController.text = state.restTime != null && state.restTime!.inMinutes > 0
  ...
  Widget _buildAutomaticSection(MachineInactivitiesState state, ThemeData theme) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text(
  ...
  Widget _buildScheduledSection(MachineInactivitiesState state, ThemeData theme) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text(
  ...
  void _onSaveAutomatic() {
    final continueCapacity = int.tryParse(_continueCapacityController.text.trim());
    if (continueCapacity == null || continueCapacity <= 0) {
      _showSnack('Ingresa un número válido de procesamientos continuos.', isError: true);
      return;
    }

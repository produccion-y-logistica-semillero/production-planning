...
class SeeProcessBloc extends Cubit<SeeProcessState> {
  ...
  void retrieveSequences() async {
    ...
    response.fold(
        (f) => emit(SequencesRetrieveFailure(
            null, state.selectedProcess, state.process)),
        (success) => emit(
            SequencesRetrieved(success, state.selectedProcess, state.process)));
  }

  void selectSequence(int id) async {
    final response = await service.getFullSequence(id);
    response.fold(
        (f) => emit(SequenceRetrieveFailure(
            state.sequences, state.selectedProcess, null)),
        (success) =>
            emit(SequenceRetrieveSuccess(state.sequences, id, success)));
  }

  void deleteSequence(int id) async {
    final response = await service.deleteSequence(id);
    response.fold(
        (f) => emit(SequenceDeletedFailure(
            state.sequences, state.selectedProcess, state.process)), (s) {

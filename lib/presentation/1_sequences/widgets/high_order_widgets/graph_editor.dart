...
class _Selection {
  ...
  const _Selection.edge(int index)
      : this._(_SelectionType.edge, edgeIndex: index);
}
...
enum _DragMode {
  none,
  draggingNode,
  draggingHandleSource,
  draggingHandleTarget
}
...
class NodeEditorState extends State<NodeEditor> {
  ...
  void _endDragHandle() {
    ...
    if (hit != null) {
      ...
      } else if (_wouldCreateCycle(newConn.source, newConn.target,
          ignoreIndex: idx)) {
          ...
          child: MouseRegion(
            onHover: (e) {
              ...
              if (isConnecting || isDraggingHandle) {
                ...
                setState(() =>
                    _cursor = null); // <-- evita que quede una guía “fantasma”
              }
            },
            child: LayoutBuilder(
  ...
  Offset? _segmentIntersection(Offset p1, Offset p2, Offset p3, Offset p4) {
    ...
    bool onSeg(Offset a, Offset b, Offset p) {
      final minX = math.min(a.dx, b.dx) - 1e-6,
          maxX = math.max(a.dx, b.dx) + 1e-6;
      final minY = math.min(a.dy, b.dy) - 1e-6,
          maxY = math.max(a.dy, b.dy) + 1e-6;
...
class _GraphPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (var i = 0; i < connections.length; i++) {
      ...
      final selected =
          selection.type == _SelectionType.edge && selection.edgeIndex == i;
    ...
    if (selection.type == _SelectionType.edge) {
      ...
      final p1 = _shrinkToRectEdge(_center(_nodeRect(e.source)),
          _center(_nodeRect(e.target)), _nodeRect(e.source));
      final p2 = _shrinkToRectEdge(_center(_nodeRect(e.target)),
          _center(_nodeRect(e.source)), _nodeRect(e.target));
    ...
    if (connectingFrom != null && cursor != null) {
      ...
      final toRect = _nodeRect(e.target);
      final from =
          _shrinkToRectEdge(_center(fromRect), _center(toRect), fromRect);
    ...
    for (final entry in nodePos.entries) {
      ...
      final isSel =
          selection.type == _SelectionType.node && selection.nodeId == id;

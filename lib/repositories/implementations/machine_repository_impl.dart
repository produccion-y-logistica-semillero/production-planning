...
class MachineRepositoryImpl implements MachineRepository {
  ...
  Future<Map<String, dynamic>> machineEntityToJson(MachineEntity entity) async {
    final proccesing =
        '${entity.processingTime.inHours.toString().padLeft(2, '0')}:${(entity.processingTime.inMinutes - (entity.processingTime.inHours * 60)).toString().padLeft(2, '0')}:00';
    final preparation = entity.preparationTime != null
        ? '${entity.preparationTime!.inHours.toString().padLeft(2, '0')}:${(entity.preparationTime!.inMinutes - (entity.preparationTime!.inHours * 60)).toString().padLeft(2, '0')}:00'
        : null;
    ...
    return {
      ...
      "preparation_time":
          preparation != null ? '1970-01-01 $preparation' : null,
      "rest_time": rest,
  ...
  Future<MachineEntity> jsonToEntity(Map<String, dynamic> map) async {
    ...
    return MachineEntity(
      ...
      processingTime: Duration(
          ...
          minutes:
              int.parse(map["processing_time"].toString().substring(14, 16))),
      preparationTime: map["preparation_time"] != null
          ? Duration(
              hours: int.parse(
                  map["preparation_time"].toString().substring(11, 13)),
              minutes: int.parse(
                  map["preparation_time"].toString().substring(14, 16)))
          : null,
      restTime: rest,

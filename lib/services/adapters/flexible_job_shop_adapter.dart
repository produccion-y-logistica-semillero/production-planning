...
class FlexibleJobShopAdapter {
  ...
  Future<Tuple2<List<PlanningMachineEntity>, Metrics>?> flexibleJobShopAdapter(
    ...
    for (final typeId in machineTypeIds) {
      final responseMachines = await machineRepository.getAllMachinesFromType(
        typeId,
      );
    ...
    // Crear el input para el algoritmo Flexible Job Shop (expandiendo por amount)
    ...
    for (final job in order.orderJobs!) {
      ...
      for (final task in job.sequence!.tasks!) {
        ...
        for (final machine in machines.where(
          (m) => m.machineTypeId == task.machineTypeId,
        )) {
          Duration processingTime = machine.processingTime;

          print(
            "processing time: $processingTime, processing units: ${task.processingUnits}",
          );
          final duration = ruleOf3(processingTime, task.processingUnits);

          if (duration == Duration.zero) {
            print(
              'Error: Duración calculada es cero para la máquina ${machine.id}',
            );
            continue;
          }
    ...
    for (final machine in machines) {
      planningMachines.add(
        PlanningMachineEntity(machine.id!, machine.name, []),
      );
    }

    for (final out in output) {
      final originalJobId = instanceToOriginal[out.jobId] ?? out.jobId;
      final job = order.orderJobs!.firstWhere((j) => j.jobId == originalJobId);
      ...
      for (final taskEntry in out.scheduling.entries) {
        ...
        print('Task Range - Start: ${timeRange.start}, End: ${timeRange.end}');
        ...
        final planningMachine = planningMachines.firstWhere(

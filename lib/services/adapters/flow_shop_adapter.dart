...
class FlowShopAdapter {
  ...
  Future<Tuple2<List<PlanningMachineEntity>, Metrics>?> flowShopAdapter(
    ...
    for (final typeId in machinesTypesIds) {
      final machinesSpecific = await machineRepository.getAllMachinesFromType(
        typeId,
      );
    ...
    final mergedMatrix = _mergeChangeoverMatrices(
      defaultMatrix,
      changeoverMatrix,
    );

    //we create the input (expand jobs by amount)
    ...
    for (final job in order.orderJobs!) {
      ...
      for (final task in job.sequence!.tasks!) {
        final machineOfTask = machines
            .where((m) => m.machineTypeId == task.machineTypeId)
            .first;
        taskTimes[task.id!] = ruleOf3(
          machineOfTask.processingTime,

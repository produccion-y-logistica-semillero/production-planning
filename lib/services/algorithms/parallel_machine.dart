...
class ParallelMachine {
  ...
  void sptaRule() {
    _schedule((a, b) {
      int minTimeA = a.durationsInMachines.values
          .reduce((x, y) => x < y ? x : y)
          .inMilliseconds;
      int minTimeB = b.durationsInMachines.values
          .reduce((x, y) => x < y ? x : y)
          .inMilliseconds;
  ...
  void eddaRule() {
    ...
    _schedule((a, b) {
      bool aAvailable = a.availableDate.isBefore(now) ||
          a.availableDate.isAtSameMomentAs(now);
      bool bAvailable = b.availableDate.isBefore(now) ||
          b.availableDate.isAtSameMomentAs(now);
  ...
  void _assignJobsToMachines() {
    ...
    for (var job in inputJobs) {
      ...
      for (var entry in job.durationsInMachines.entries) {
        ...
        Duration delay = endTime.isAfter(dueDate)
      ...
      if (bestMachineId != -1) {
        machineAvailable[bestMachineId] =
            bestEndTime; // Actualiza disponibilidad

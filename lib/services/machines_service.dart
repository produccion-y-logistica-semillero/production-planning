...
class MachinesService {
  ...
  Future<Either<Failure, MachineTypeEntity>> addMachineType(
    ...
    return response.fold((f) => Left(f), (id) {
      machine.id = id;
      return Right(machine);
    });
  }

  Future<Either<Failure, MachineEntity>> addMachine(
      int machineTypeId,
      String name,
      String? status,
  ...
  MachineStandardTimes getStandardTimesForType(int machineTypeId) {
    return _standardTimesCache[machineTypeId] ??
        MachineStandardTimes.defaults();
  }

  void updateStandardTimesForType(
  ...
  Future<Either<Failure, MachineInactivityEntity>> addMachineInactivity(

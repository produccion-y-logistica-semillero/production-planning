...
void main() {
  test('get machine type use case', () async {
    ...
    when(() => mockRepository.getAllMachineTypes())
        .thenAnswer((_) async => Right(machineList));

    //do
    final result = await mockRepository.getAllMachineTypes();

    //assert
    expect(result, equals(Right(machineList)));
    verify(() => mockRepository.getAllMachineTypes())
        .called(1); //make sure that it was called exactly once
  });
}
